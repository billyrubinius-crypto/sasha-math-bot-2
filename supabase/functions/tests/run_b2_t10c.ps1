<#
  ...
  ВНИМАНИЕ (T10-10C2, миграция 045): контракт действия mock_exam_upsert изменён — пригодный
  результат (целый балл 0-100 + дата) теперь уходит в canonical weekly_mock_exams, а в архиве
  появляется зеркало под именем «Недельный пробник ДД.ММ.ГГГГ». Поэтому кейсы S05/06/07 и сверки
  V5-V8 этого скрипта описывают УСТАРЕВШЕЕ поведение и после миграции 045 дадут FAIL.
  Актуальная проверка пробников — run_b2_t10c2.ps1; остальные кейсы (секрет, lookup, группа,
  оплата, запрещённые поля) остаются в силе.
#>
param(
  [string]$FunctionUrl,
  [switch]$ShowPrepSql,
