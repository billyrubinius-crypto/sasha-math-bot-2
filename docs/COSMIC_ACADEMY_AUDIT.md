...
### 10.3. Экипированный фон (слот `background`)

**Источник:** `student_equipment` slot `background` → `eq.background.payload`
(`bg-grid` | `bg-space` | `bg-aurora` | `bg-draft`).

#### 10.3.1. Как сделано сейчас — и почему это ограничение старой реализации
...
**Купленный фон виден только на одном экране из пяти** — на `#screen-profile`. Стоит ученику
переключиться на «Домашку», «Лидеры», «Магазин» или «Ещё» — оплаченный предмет исчезает.
Это **ограничение старой реализации**, а не сознательное продуктовое решение, и оно
**устраняется в рамках редизайна Cosmic Academy** (этап 1).

#### 10.3.2. Целевое поведение Cosmic Academy

- экипированный предмет слота `background` меняет фон **всего ученического Mini App**;
- фон виден на всех пяти экранах: Профиль, Домашка, Лидеры, Магазин, Ещё;
- переключение вкладок не сбрасывает фон;
- фон находится **за** карточками и контентом;
- карточки, поля ввода, модальное окно и нижняя навигация сохраняют **собственные контрастные
  поверхности** (фон не должен «просвечивать» сквозь них до потери читаемости);
- после `equip`, `unequip` или смены предмета фон обновляется **сразу**, без полной перезагрузки
  Telegram Mini App;
- после обычной перезагрузки экипированный фон восстанавливается из существующего equipment;
- при отсутствии экипированного фона используется **базовый фон Cosmic Academy**;
- на профиле **не должно быть двойного наложения** одного и того же пользовательского фона.

#### 10.3.3. Что сохраняется без изменений

Слот `background`; `BG_CLASSES`; классы `bg-grid`, `bg-space`, `bg-aurora`, `bg-draft`;
`item_code`; `render_payload`; существующие RPC; структура Supabase; equipment map
(`equipmentQuery` / `buildEquipMap`); whitelist безопасности.

**Нельзя менять:** получение equipment, серверные запросы, формат данных, проверку whitelist,
логику покупки, логику экипировки.

**Допустимо:** минимально изменить **визуальную часть** `applyProfileCosmetics` (то, к какому
элементу вешается класс фона) либо вынести применение фона в **отдельную UI-функцию**, вызываемую
оттуда же.

#### 10.3.4. Обязательное условие — снять старое применение фона к профилю

Одновременно с добавлением глобального слоя **необходимо убрать применение классов `bg-*` к
`#screen-profile`** (и соответствующие правила `#screen-profile.bg-*` из CSS). Если оставить оба
механизма, один и тот же купленный фон отрисуется дважды — глобальным слоем и поверх него слоем
профиля: получится удвоенная плотность сетки/линейки, «шов» на границе экрана и разная яркость
профиля относительно остальных четырёх экранов.

Специфичность здесь — отдельная ловушка: `#screen-profile.bg-*` — это (1,1,0), такое правило
**не перебивается** ничем классовым, поэтому «погасить» старый слой новым CSS на классах не
получится — его нужно именно удалить.

**Риск R3 (переформулирован):** это риск **переноса пользовательского фона на глобальный слой** —
потеря видимости купленного предмета, двойной слой на профиле, срыв фона при переключении вкладок
или при `themeChanged`, нечитаемый контент поверх фона. См. §19, R3.
...
4. **История коммитов:** попытка нарисовать превью иначе уже была и была откачена —
   ...
   Причина отката в самом репозитории не зафиксирована.

### 11.1. Статус R13: повышенный риск, а не блокировка

Откаченный редизайн превью (`bb89030` → `a26a98a`) **не является причиной для запрета нового
редизайна**. Правила работы с ним на этапе 7:

- перед реализацией **изучить diff откаченного решения** (`git show bb89030`, `git show a26a98a`);
- **не восстанавливать старый коммит целиком** (`git revert`/`cherry-pick` откаченного — запрещено);
- **не повторять выявленные в нём проблемные решения** — как минимум:
  - перевод превью со строкового `innerHTML` вместо DOM-пути `createElement` + `textContent`
    (в `bb89030` почти все ветки `shopPreview` стали `p.innerHTML = '<span …>'`);
  - сопоставление фонов по `item_code` с ключами в другом регистре разделителя
    (`bg_grid`/`bg_space`/… против фактических `bg-grid`/`bg-space`/…) — такое сопоставление
    молча промахивается и даёт fallback-градиент вместо реального фона;
  - вытаскивание текста титула регуляркой из `item.name` внутри превью;
  - захардкоженные в JS градиенты фонов, дублирующие CSS-классы `bg-*`;
- выполнить **новый `shopPreview` по требованиям Cosmic Academy** (§11.2).

### 11.2. Требования Cosmic Academy к `shopPreview` (этап 7)

Превью строится **по фактическим слотам**:

| Слот | Что показывает превью |
|---|---|
| `name_color` | пример ника **настоящим цветом**; `gold` — золотым градиентом |
| `frame` | компактный демонстрационный аватар с **реальной рамкой** через безопасное сопоставление `render_payload` |
| `background` | **настоящая миниатюра** фона: `bg-grid`, `bg-space`, `bg-aurora`, `bg-draft` |
| `title` | компактный пример титула |
| `crown` | единый знак короны |
| `status_emoji` | **настоящий emoji товара** |
| `shield` (item_kind) | единый знак щита |

**Обязательно сохранить:** безопасный DOM-путь через `createElement` и `textContent`; whitelist
(`FRAME_CLASSES`, `BG_CLASSES`, `/^#[0-9a-fA-F]{6}$/`); `render_payload`; `item_code`; слоты;
`item_kind`; RPC; покупку; экипировку; снятие; состояния баланса и доступности.
...
- `#screen-profile.bg-*` (4 правила) — (1,1,0), перебивает всё классовое; **на этапе 1 эти
  правила удаляются вместе со старым применением фона к профилю, фон переезжает на глобальный
  слой приложения** (см. 10.3);
...
1. **`.my-hw-item.status-pending` / `.status-approved` / `.status-rejected` — не применяются
   ...
   **`status-submitted` и `status-checked`**, для которых стилей нет: цветная левая полоска
   ...
   а не в `status`.
   **Решение принято:** чинится на этапе домашки (этап 5) — в CSS добавляются стили фактически
   создаваемых классов `status-submitted` и `status-checked`. Значения данных и бизнес-логика
...
| R3 | **Перенос пользовательского фона на глобальный слой** | Купленный фон пропадает на части экранов, срывается при переключении вкладок или `themeChanged`, рисуется дважды на профиле (если не снять старое `#screen-profile.bg-*`), либо делает контент нечитаемым | §10.3, §13.3 |
...
| R13 | **Повышенный (не блокирующий):** новый `shopPreview` повторяет проблемные решения откаченного `bb89030` | Повторный откат работы этапа 7; переход на `innerHTML`, промах сопоставления фонов, потеря whitelist | §11.1, §11.2 |
| R14 | Работа от локального `main` (`fb68ea8`) вместо `origin/main` (`e5f675b`) | Редизайн ляжет на код 19-коммитной давности | §0.2 |
| R15 | `.my-hw-item.status-*` не работает уже сейчас | **Решено чинить на этапе 5**: добавить CSS для фактических `status-submitted`/`status-checked`; данные и бизнес-логику не трогать | §13.6 |
...
## Состояние после редизайна Cosmic Academy

- **Дата проверки:** 27 июля 2026 года
- **Ветка:** `feat/cosmic-academy`
- **Проверенный диапазон этапов 7–8:** `2b195b2..a0402cb`
- **Итоговый диапазон с исправлениями и этапом 9:** `2b195b2..HEAD`

### Состав проверенного редизайна

Проверены этапы 1–8: токены и Telegram theme, SVG-спрайт и навигация, профиль и неделя,
квесты/пробники/щиты/история, домашка, лиги, магазин и персональный титул, FAQ/состояния/
адаптивность. Отдельно учтён исправляющий проход светлой темы
`2b195b2 fix(ui): strengthen Cosmic Academy light theme identity`.

Независимое ревью этапов 7–8 проводилось по фактическим diff:

- этап 7 — `444464d`;
- этап 8 и исходный HEAD ревью — `a0402cb`;
- база непосредственно перед этапом 7 — `2b195b2`.

### Результаты независимого ревью этапов 7–8

Исправляющий коммит `2593848 fix(ui): address Cosmic Academy stages 7-8 review findings`
устранил подтверждённые дефекты:

- горизонтальный overflow и рассинхронизацию ширины `.screen`/`.bottom-nav` на 480 px и
  desktop;
- блокировку пользовательского zoom в viewport;
- отсутствие явных правил `ca-state--loading`/`ca-state--empty` и ошибочное пустое состояние
  ошибки квестов;
- недостаточный touch target кнопок покупки/экипировки и emoji-chip;
- отсутствие safe-area/вертикальной прокрутки у модалки персонального титула;
- пустую карточку коллекции при бандле без отрисованных товаров;
- пустую непустую ячейку showcase для неизвестного устаревшего предмета.

`shopPreview` сохранил DOM-путь `createElement`/`textContent`, whitelist `FRAME_CLASSES` и
`BG_CLASSES`, не определяет тип косметики по частям `item_code`, не мутирует входные объекты и
не повторяет отменённый подход `bb89030`. Secure/legacy ветвление, RPC покупки/экипировки/
титула/коллекции, цены, лимит титула 3–24 и порядок обновления магазина не менялись.

### Дополнительные результаты этапа 9

Глобальная регрессия выявила и устранила три локальных DOM-риска:

- аватар теперь создаётся DOM-методами, а не строкой `innerHTML`; добавлен безопасный `alt`;
- подпись выбранной точки пробника выводится через `textContent`;
- серверный `replacements_left` экранируется перед попаданием в строковый атрибут квеста.

Контрактный скрипт актуализирован: устранённые долги `wd-assigned`,
`status-submitted` и `status-checked` удалены из `KNOWN_MISSING_CLASSES` и переведены в
обязательные проверки; добавлены критичные состояния этапов 7–8, preview-классы магазина и
составные модификаторы ошибок, магазина, фонов и архива домашки.

### Подтверждённые проверки

- `python scripts/check_ui_contract.py` — exit code 0: ID, inline-handlers, динамические классы
  и составные модификаторы соответствуют итоговому DOM/CSS.
- `node --check shared.js` и все `js/student-*.js` — успешно.
- 98 статических DOM ID уникальны; `exam-info-box` подтверждён как единственный требуемый
  динамический ID.
- Порядок пяти экранов и пяти кнопок навигации сохранён; student script order не изменён,
  `shared.js` остаётся в `<head>`, ES modules/IIFE не добавлены.
- Статический и VM-прогон подтвердил график для одной, нескольких и одинаковых точек без
  `NaN`/`Infinity`, безопасный аватар и экранирование строк квеста.
- Локальный Chromium подтвердил переключение пяти экранов, табов домашки и лидерборда,
  открытие/закрытие FAQ, закрытие модалки кнопкой и кликом по фону, безопасный preview титула,
  Unicode/кириллицу и границы 3–24. В консоли нет `SyntaxError`, `ReferenceError`,
  `TypeError`, `error` или `warn`.
- Принудительно проверены светлая и тёмная темы. Усиленная светлая тема сохранена; тёмные
  карточки, модалка и навигация читаемы.
- Проверены ширины 320, 360, 375, 390, 412, 430, 480, 768 и 1024 px:
  горизонтального overflow нет; на desktop `.screen` и `.bottom-nav` центрированы и ограничены
  480 px.
- Проверены базовый Cosmic Academy background и whitelist-фоны `bg-grid`, `bg-space`,
  `bg-aurora`, `bg-draft`: каждый создаёт собственный слой и не меняет ширину документа.
- Viewport оставлен в доступном виде:
  `width=device-width, initial-scale=1.0, viewport-fit=cover`; `maximum-scale=1.0` и
  `user-scalable=no` удалены.

### Ограничения локальной проверки

Вне Telegram отсутствуют валидный `initData`, JWT и dev-данные, поэтому локальный браузер
ожидаемо показывает «Ошибка доступа». Реальные RPC покупки, экипировки, снятия косметики,
отправки/повторной отправки титула, бонуса коллекции, showcase persistence, щитов, квестов,
загрузки домашки и серверных лиг не вызывались. Эти пути подтверждены статическим сравнением,
контрактными и синтаксическими проверками, но не объявляются end-to-end проверенными.

Инструментальный Tab/Enter не воспроизвёл системную клавиатурную навигацию WebView; при
фокусе native `summary` подтверждены `:focus-visible` и 2 px outline, а семантика
`details/summary`, `button`, `select` сохранена. Реальный screen reader и Telegram WebView на
устройстве не тестировались.

Teacher app, backend, SQL, Supabase, RPC и `js/student-auth.js` не изменялись. Ветка не
переключалась, merge/rebase/push не выполнялись.
