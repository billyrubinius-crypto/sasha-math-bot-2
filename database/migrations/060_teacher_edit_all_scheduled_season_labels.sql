-- Season V2: каждая запланированная строка имеет редактируемые публичные номер и название.
-- sequence_no/season_type остаются внутренними полями календаря и не показываются учителю.

begin;

-- Номер является публичной подписью, а не идентификатором. Учитель может повторить номер
-- или временно создать дубликат при перенумерации нескольких будущих сезонов.
drop index if exists public.idx_seasons_v2_display_number;

update public.seasons s
   set display_number = p.sequence_no
  from public.season_v2_presets p
 where s.preset_code = p.preset_code
   and (
     s.display_number is null
     or s.display_number = p.competition_season_no
   );

comment on column public.seasons.display_number is
  'Редактируемый учителем публичный номер любого запланированного сезона; не влияет на внутренний порядок календаря.';
    ...
    from (
      select p.sequence_no,
             jsonb_build_object(
               ...
               'display_number', coalesce(s.display_number, p.sequence_no),
...
begin
  ...
  if p_display_number is null or p_display_number not between 1 and 999 then
